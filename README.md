...
| **LangGraph** | **没用** | 见 [§5.3](#53-为什么不用-langgraph-与多智能体) |
| **多智能体协作** | **没用** | 同上，单 agent 足够且更可控 |
| **MCP** | **没接** | 见 [§5.6](#56-mcp-目前没接) |
| **Few-shot** | **没用** | 靠工具 schema 的 description 约束格式，比塞示例更省 token |
...
## 5.3 为什么不用 LangGraph 与多智能体

**LangGraph 没用。** 它的价值在于把复杂状态机显式化。但本项目的控制流实测只需要
一个 while 循环加三重预算：步数 14 / token 60000 / 耗时 90 秒。引入图框架后，
预算、轨迹、工具拉黑、降级这些我最需要精细控制的东西都要绕框架的抽象走，
改一行行为得先读框架源码。取舍写在 [§4.1](#41-技术栈)。

**多智能体也没用。** 常见范式（Planner-Executor、辩论、角色分工）在这里收益为负：

- 购物决策的子任务（比价、成分分析、排序）**没有真正的并行冲突**，
  拆成多个 agent 只是把工具调用换成 agent 间通信，多一层序列化开销和失败点
- 结论必须**唯一且可解释**。多 agent 辩论出的折中结论，很难回答
  「为什么这瓶排第一」——而这恰好是本项目的核心交付物
- 硬禁忌（糖尿病不推高糖）要求**确定性过滤**，不能是协商结果

真需要横向扩展时，正确的做法是加工具，而不是加 agent。
...
**没接。** 当前 17 个工具都是进程内 Python 函数，没有跨进程工具协议。
对单体应用来说 MCP 的收益（工具跨宿主复用、第三方工具即插即用）还不抵它的
额外复杂度。

**要接的话改动面很小**，因为工具层已经收敛成「schema + handler」两部分：

- `tool_schemas()` 产出的 JSON Schema 与 MCP 的 `tools/list` 格式高度同构
- `execute_tool()` 就是 `tools/call` 的分发器
- 所以接 MCP 等于在 `toolkit.py` 外面包一层 server，
  再让 `build_llm()` 支持把远端工具合并进本地工具表

真正会接的场景是：想把「比价」「成分分析」开放给别的 agent 复用，
或者想引入外部 MCP 工具（如真实电商查询）而不想写适配器。

## 5.7 Prompt 策略：为什么不用 Few-shot

只有一个 `SYSTEM_PROMPT`（在 `llm.py`），内容是决策纪律而非话术：
...
- 购物场景的输入分布太广（六品类 × 各种表达），几个示例覆盖不了，
  反而容易让模型往示例的品类上靠
- 品类专属话术全在 `app/catalog/` 配置里，由工具返回值携带，
  新增品类不用改 prompt

动态部分由 `context.py` 装配：系统指令 → 长期记忆摘要 → 历史对话 →
